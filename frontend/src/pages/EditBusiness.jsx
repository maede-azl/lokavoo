...
import "./AddBusiness.css"; // همان استایل افزودن کسب‌وکار
...
/* ============ ICON MAP ============ */
const ICON_PATHS = {
  bakery: <path d="M4 12a8 8 0 0 1 16 0v6H4v-6Z" />,
  cafe: (
    <>
...
export default function EditBusiness({ onBack }) {
  const navigate = useNavigate();
  const { id } = useParams(); // /edit-business/:id
  ...
  const [activeNav, setActiveNav] = useState("edit-business");

  useEffect(() => {
    setActiveNav(sellerMenuItem?.key || "edit-business");
  }, [sellerMenuItem?.key]);
  ...
  // تصاویر موجود (از سرور) + تصاویر جدید (File)
  ...
  useEffect(() => {
    if (!id) {
      setLoadError("شناسه کسب‌وکار مشخص نیست.");
    ...
    fetch(`${API_BASE}/api/businesses/${id}/settings`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then((res) => res.json())
      .then((data) => {
        if (!data.success || !data.data) {
          setLoadError(data.message || "اطلاعات یافت نشد.");
          return;
        }
        const b = data.data;
        ...
        // ساعت کاری
        if (b.opening_time) {
          const [oh, om] = b.opening_time.split(":");
          setOpenHour(oh || "08");
          setOpenMinute(om?.slice(0, 2) || "00");
        }
        if (b.closing_time) {
          const [ch, cm] = b.closing_time.split(":");
          setCloseHour(ch || "21");
          setCloseMinute(cm?.slice(0, 2) || "00");
        }
        ...
        if (b.latitude != null && b.longitude != null) {
          ...
          setMarkerPos({ lat: Number(b.latitude), lng: Number(b.longitude) });
          setMarkerAddress(b.address || "");
          setMarkerCity(b.city || "");
          setMarkerProvince(b.province || "");
        }

// موجودی برای آینده
if (Array.isArray(b.images) && b.images.length) {
  const imgs = b.images.map((img, idx) => {
    const rawUrl = typeof img === 'string' ? img : (img.url || img.image_url || img.path || '');
  ...
  setExistingImages([]);
      ...
      .catch((err) => {
        console.error(err);
        setLoadError("خطا در دریافت اطلاعات کسب‌وکار");
      })
      .finally(() => setLoadingBusiness(false));
  }, [id]);
  /* ---------- object URLs برای تصاویر جدید ---------- */
  useEffect(() => {
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    if (data.success) {
      ...
      setErrorMsg(data.message || data.errors?.[0]?.message || "خطا در ذخیره تغییرات");
    }
  } catch (err) {
    console.error(err);
      ...
      <div className="addbiz-root" dir="rtl" lang="fa">
        <div className="success-wrap">
          <div className="success-card">
            ...
            <h1>ذخیره شد!</h1>
            <p>تغییرات کسب‌وکار «{shopName}» با موفقیت ذخیره شد.</p>
  ...
  if (loadingBusiness) {
    return (
      <div className="addbiz-root" dir="rtl" lang="fa">
        <div className="success-wrap">
          <div className="success-card">
            <p style={{ color: "var(--text-muted)" }}>در حال بارگذاری اطلاعات کسب‌وکار...</p>
          </div>
        </div>
      </div>
    );
        ...
        <div className="main">
          <section className="page-head">
            ...
            <div className="page-head-left">
              ...
              <div>
                <h1>ویرایش کسب‌وکار</h1>
                <p>اطلاعات مغازه یا خدمات خود را به‌روزرسانی کنید</p>
              </div>
            </div>
          ...
          <div className="form-shell">
            <div className="form-main">
              ...
              <div className="form-card" style={{ animationDelay: ".15s" }}>
                ...
                >
                  ...
                  <p>عکس‌های جدید را بکشید و رها کنید</p>
                ...
                <div className="thumb-grid">
                  {visibleExisting.map((img) => (
                    <div className="thumb" key={`ex-${img.id}`}>
                      <img src={img.url} alt="existing" />
                      <span className="thumb-remove" onClick={() => removeExistingImage(img.id)}>
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3">
                  ...
                  {newImageUrls.map((url, i) => (
                    <div className="thumb" key={`new-${i}`} style={{ animationDelay: i * 0.04 + "s" }}>
                      <img src={url} alt="preview" />
                      <span className="thumb-remove" onClick={() => removeNewImage(i)}>
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3">
                          <path d="M18 6 6 18M6 6l12 12" />
                        </svg>
                      </span>
            ...
            <div className="preview-col">
              ...
              <div className="shop-card">
                ...
                >
                  {(newImageUrls[0] || visibleExisting[0]?.url) && (
                    <img src={newImageUrls[0] || visibleExisting[0]?.url} alt="" />
                  )}
                  <svg
                    className="cat-glyph"
                    viewBox="0 0 24 24"
                    ...
                    stroke="currentColor"
                    strokeWidth="1.6"
                    style={{ opacity: newImageUrls.length + visibleExisting.length > 0 ? 0 : 1 }}
                  >
              ...
              <div className="preview-note">
                ...
                کارت بالا دقیقاً همانی است که مشتری‌ها در نتایج جستجو می‌بینند — با تغییر فیلدها، آن را زنده مشاهده کنید.
              </div>
            </div>
          </div>
        </div>
